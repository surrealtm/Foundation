...
#if FOUNDATION_WIN32
    Window_Win32_State *win32 = (Window_Win32_State *) window;
...
void destroy_window_buffer(Window_Buffer *buffer) {
    Default_Allocator->deallocate(buffer->buffer);
    buffer->buffer = null;
    buffer->w = 0;
    buffer->h = 0;
}
...
void blit_pixels_to_window(Window *window, u8 *buffer, s32 w, s32 h, u8 channels) {
    ...
    if(!SetDIBitsToDevice(win32->dc, 0, 0, w, h, 0, 0, 0, h, buffer, &bmi, DIB_RGB_COLORS)) {
...
u8 *convert_bgra_to_rgba(u8 *input, s32 w, s32 h) {
    ...
    for(s64 y = 0; y < h; ++y) {
        for(s64 x = 0; x < w; ++x) {
            s64 offset = (y * w+ x) * 4;
            result[offset + 0] = input[offset + 2];
            result[offset + 1] = input[offset + 1];
            result[offset + 2] = input[offset + 0];
...
u8 *convert_rgba_to_bgra(u8 *pixels, s32 w, s32 h) {
    // The underlying operation is actually exactly the same, we just swap the first and third channel both
    // times.
    return convert_bgra_to_rgba(pixels, w, h);
}

u8 *convert_window_buffer_to_rgba(Window_Buffer *buffer) {
    return convert_bgra_to_rgba(buffer->buffer, buffer->w, buffer->h);
}
